document.addEventListener("DOMContentLoaded", function () {
  ...
  let stream;
  let absensiData = []; // Menyimpan data absen
  ...
  async function posisiAnda(position) {
    ...
    let BatasLokasiAccess = [
      ...
      { lat: -6.97086, lng: 110.018706 }, // titik batas lokasi yang di izinkan 3. -6.970860,110.018706
      ...
      preConfirm: async (nama) => {
        ...
        if (!allowedNames.includes(nama))
          return Swal.showValidationMessage(
            "Nama Tidak Terdaftar Silahkan Hubungi Mas Pur!"
          );
        return nama;
      },
      ...
      if (result.isConfirmed) {
        ...
        const tanggal =
          document
            ...
            ?.textContent?.trim()
            ?.substring(0, 50) || "Tanggal Tidak Ditemukan";
        ...
        Swal.fire({
          ...
          const telegramBotToken =
            "7079092015:AAFOhQM0L0PGWmKcfW2DULtjo0KHzBEHbz8";
            ...
            if (data.ok) {
              ...
              if (typeof saveToExcel === "function") {
                ...
                saveToExcel(
                  ...
                  tanggal,
                  jam,
                  photoPreview?.src || "Foto tidak ditemukan"
                );
              } else {
              ...
              console.warn(
                `Peringatan: Telegram API mengembalikan error -> ${data.description}`
              );
              Swal.fire({
                icon: "warning",
  ...
  async function sendDataExcelTele() {
    try {
      ...
      const fileName = `Absensi_${now.toLocaleString("id-ID", {
        month: "long",
        year: "numeric",
      })}.xlsx`;
      let wb = XLSX.utils.book_new();
      ...
      let fileBlob = new Blob([XLSX.write(wb, { type: "array" })], {
      ...
      const response = await fetch(
        `https://api.telegram.org/${API}/sendDocument`,
        {
          method: "POST",
          body: formData,
        }
